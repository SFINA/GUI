...
public class SfinaNetworkReader {
    ...
    public Vector<FlowNetwork> readNetworks(){
        ...
        if (chooser.showOpenDialog(null) == JFileChooser.APPROVE_OPTION) { 
            ...
            SelectItemComboBoxDialog dlg_peers = new SelectItemComboBoxDialog("Select any one peer to explore networks.");
            ...
            if(cBoxDlg == JOptionPane.YES_OPTION){
                ...
                if(cBoxDlg_outputs == JOptionPane.YES_OPTION){
                    String[] times = new File(path + File.separatorChar + peers[0] + File.separatorChar+outputs[0]).list(new FilenameFilter(){
                        @Override
                        public boolean accept(File file, String name){
                            return new File(file, name).isDirectory();
                        }
